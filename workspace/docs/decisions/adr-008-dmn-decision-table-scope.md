...
- Exactly one `<decisionTable>` per DMN XML document — no decision
  requirement graph, no decision-to-decision dependencies. `DmnParser`
  throws if it finds zero or more than one, rather than silently
  picking one.
